...
def openImages():
    ...
    def displayImage(image_path):
        ...
        # Calculate the new dimensions based on the window size
        # # Replace with the actual new window height
...
def editRow():
    ...
    def saveChanges():
        ...
        updated_values = [entry.get() if isinstance(entry, CTkEntry) else entry.get() for entry in entries]
    ...
    save_button = CTkButton(edit_window, text="Save", command=saveChanges)
    save_button.grid(row=len(tree["columns"]), column=0, padx=10, pady=10)

    cancel_button = CTkButton(edit_window, text="Cancel", command=cancelChanges)
...
# Control panel
panel = CTkFrame(root)
# Add the forest-dark.tcl only to the treeview
panel.grid(row=0, column=0)

# Refresh button
refreshBtn = CTkButton(panel, text="Refresh")
refreshBtn.grid(row=0, column=0, padx=20, pady=10)
refreshBtn.configure(command=refreshDb)

# Delete row button
delBtn = CTkButton(panel, text="Delete row")
...
editBtn = CTkButton(panel, text="Edit row")
editBtn.grid(row=0, column=2, padx=20, pady=10)
editBtn.configure(command=editRow)

# Show images button
imagesBtn = CTkButton(panel, text="Show images")
...
tree.tk.call("source", "forest-dark.tcl")
style.theme_use("forest-dark")

# Create a vertical scrollbar
scrollbar = CTkScrollbar(root, command=tree.yview)
